...
const double initBoxX = 100, initBoxY = 100;
const int simulations = 100;
const double d = 1;
bool sighted = 1;
bool truncateObject = 1;
bool invisible = 0;
bool objectSensor = 1;
double dTheta = asin((2*birdRad + d)/v0); //angle that birds can change their orientation by in a timestep.
double movesRO[nMovesRO] = { -dTheta, dTheta }; //possible moves.
double movesSC[nMovesSC] = { v0 - dv - d, v0, v0 + dv + d};
...
//function prototypes
bool birdCollision(int i, int nFut, Bird *birds, double xi, double yi, double oi, double vi);
bool objectCollision(double xi, double yi);
...
bool runSim(int simNum, bool sighted, bool truncateObject, bool truncateBird, bool invisible);

int main(int, const char * const[])
{
	for (int simNum = 1; simNum<= simulations; simNum++){
		for (bool sighted: {true, false}){
			for (bool truncateObject: {true, false}){
				for (bool invisible: {true, false}){
					for (bool objectSensor: {true, false}){
						runSim(simNum, sighted, truncateObject, invisible, objectSensor);
					}
				}
			}
		}
...
bool runSim(int simNum, bool sighted, bool truncateObject, bool invisible, bool objectSensor){
	...
	namestring << "Text Files/simNum=" << simNum << "outputN=" << nBirds << "depth=" << nF <<  "sighted=" << sighted << "truncateObject=" << truncateObject
	<<  "invisible=" << invisible << "objectSensor" << objectSensor << ".txt";
	...
	for (int i = 0; i<nBirds; i++) {
		birdList[i].set_position(initBoxX*uniformReal(generator), initBoxY*(uniformReal(generator)) + nF * v0);
		birdList[i].update_Orientation(normalDist(generator)*dTheta - M_PI/4);
	}
	...
	for (int ts = 0; ts<nSteps; ts++) {
		...
		for (int bn = 0; bn < nBirds; bn++) {
			...
			for (int nFut = 1; nFut < nF + 1; nFut ++){
				futurePositions[bn][nFut][0] = futurePositions[bn][nFut -1][0] + futurePositions[nFut - 1][0][3] * dt*cos(futurePositions[nFut - 1][0][2]);
				futurePositions[bn][nFut][1] = futurePositions[bn][nFut -1][1] + futurePositions[nFut - 1][0][3] * dt*sin(futurePositions[nFut - 1][0][2]);
				futurePositions[bn][nFut][2] = futurePositions[bn][nFut -1][2];
				futurePositions[bn][nFut][3] = futurePositions[bn][nFut -1][3];
			}
		}
		...
		for (int bn = 0; bn<nBirds; bn++) {
			...
			vector <int> indicies(1,0);
			int maxVal = uniqueVisStates[0];
			for (int h = 1; h<nMovesTot; h++) {

				if (uniqueVisStates[h] > maxVal){
					indicies.assign(1,h);
					maxVal = uniqueVisStates[h];
				} else if (uniqueVisStates[h] == maxVal){
					indicies.push_back(h);
				}
			}
			shuffle(indicies.begin(), indicies.end(), generator);

			int maxInd = indicies[0];
	...
	// cout << "simNum=" << simNum << "outputN=" << nBirds << "depth=" << nF <<  "sighted=" << sighted << "truncateObject=" << truncateObject
	// <<  "invisible=" << invisible << "objectSensor" << objectSensor << " completed" << endl;;
...
void getVisualState(Bird *birdList, int nFut, int i, double cX, double cY, double cAng) {
	...
	for (int j = 0; j<nBirds; j++) {
		...
		if (invisible && objectCollision(futurePositions[j][nFut][0], futurePositions[j][nFut][1])) continue; //the bird cannot see the birds in the "fog"
...
void updateTree(double exploreX, double exploreY, double exploreO, double exploreV, Bird *bird, int bn, int nFut) {
	...
	if (birdCollision(bn, nFut + 1, bird, exploreX, exploreY, exploreO, exploreV)) return;

	if (truncateObject && objectCollision(exploreX, exploreY)) return;

	// cout << exploreX << " " << exploreY << endl;

	if (sighted || not objectCollision(exploreX, exploreY)) getVisualState(bird, nFut + 1, bn, exploreX, exploreY, exploreO);
